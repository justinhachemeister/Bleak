    ...
    internal class PeParser : IDisposable
    {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    if (_peHeaders.FileHeader.Machine == Enumerations.MachineType.X86)
                    {
                        ...
                        {
                            importedFunctions.Add(new ImportedFunction(importDescriptorName, null, firstThunkOffset, (ushort) (importedFunctionThunk.Ordinal & 0xFFFF)));
                        }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            importedFunctions.Add(new ImportedFunction(importDescriptorName, importedFunctionName, firstThunkOffset, null));
                        }
        ...
        internal IEnumerable<TlsCallback> GetTlsCallbacks()
        {
            ...
            if (_peHeaders.FileHeader.Machine == Enumerations.MachineType.X86)
            {
                ...
                {
                    // Calculate the offset of the offset the TLS callback

                    var tlsCallbackRva = Marshal.PtrToStructure<uint>(_dllBuffer.AddOffset(tlsCallbacksOffset + (uint) (sizeof(uint) * tlsCallbackIndex)));
            ...
            else
            {
                ...
                for (var tlsCallbackIndex = 0; ; tlsCallbackIndex += 1)
                {
                    ...
                    var tlsCallbackRva = Marshal.PtrToStructure<ulong>(_dllBuffer.AddOffset(tlsCallbacksOffset + (ulong) (sizeof(ulong) * tlsCallbackIndex)));
